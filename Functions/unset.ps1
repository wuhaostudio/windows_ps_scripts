function unset {
    ...
    foreach ($arg in $Arguments) {
        switch -Wildcard ($arg) {
            '--help' {
                Write-Host @"
用法: unset 变量名 [变量名...] [--User] [--Process] [--Machine]
...
"@
            ...
            default {
                if ($arg -match '=') {
                    Write-Error "无法识别的参数: '$arg'。请只传入变量名，例如: unset MY_VAR"
                    return
                }
                $Names += $arg
            }
    ...
    if ($Names.Count -eq 0) {
        Write-Error "必须指定变量名，例如: unset MY_VAR"
        return
    }
...
Set-Alias -Name unexport -Value unset -Scope Global
